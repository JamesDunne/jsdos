...
// pointer to a function accepting one argument of type long and returning long value
typedef int64_t (* plfll)(int64_t, int64_t);
...
//     kprint("\n\n");
    ...
    // generate a 'mul' function:
    jit_prolog(p, &mul);
    jit_declare_arg(p, JIT_SIGNED_NUM, sizeof(int64_t));
    jit_declare_arg(p, JIT_SIGNED_NUM, sizeof(int64_t));
    jit_getarg(p, R(0), 0);
    jit_getarg(p, R(1), 1);
    jit_mulr(p, R(2), R(0), R(1));
    jit_retr(p, R(2));
    jit_generate_code(p);

    hw_txt_set_color(0x07); kprint("mul(1, 10): ");
    hw_txt_set_color(0x0F); kprint(txt_format_uint64(decfmt, 10, mul(1, 10)));
    kprint("\n");
    hw_txt_set_color(0x07); kprint("mul(2, 10): ");
    hw_txt_set_color(0x0F); kprint(txt_format_uint64(decfmt, 10, mul(2, 10)));
    kprint("\n");
    ...
    // Prepare a function call to 'mul':
    jit_prepare(p);
    jit_putargr(p, R(19));
    jit_putargi(p, 20);
    jit_call(p, mul);
    ...
    // returns from the function and returns the value stored in the register R(20)
    jit_retr(p, R(20));
    ...
    size_t pos;
    hw_txt_set_color(0x07); kprint("foo(  1): ");
    hw_txt_set_color(0x0F); kprint(txt_format_uint64(decfmt, 10, foo(1)));
    kprint("\n");
    hw_txt_set_color(0x07); kprint("foo(100): ");
    hw_txt_set_color(0x0F); kprint(txt_format_uint64(decfmt, 10, foo(100)));
    kprint("\n");
    hw_txt_set_color(0x07); kprint("foo(255): ");
    hw_txt_set_color(0x0F); kprint(txt_format_uint64(decfmt, 10, foo(255)));
    kprint("\n\n");
    ...
    // cleanup the jitter's used memory.
    jit_free(p);
